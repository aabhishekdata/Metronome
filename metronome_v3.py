...
class MetronomeApp(App):
    ...
    def build(self):
        ...
        self.bpm_input = TextInput(
            text=str(self.bpm), input_filter="int", multiline=False)
        ...
        timer_label = Label(text=self.timer_text,
                            font_size=14, size_hint_y=0.1)
    ...
    def start_metronome(self, _):
        if not self.metronome_running:
            ...
            self.metronome_thread = threading.Thread(
                target=self.run_metronome, daemon=True)
    ...
    @mainthread
    def change_color(self, box, color):
        box.color = color
    ...
    def run_metronome(self):
        while self.metronome_running:
            ...
            for i in range(4):
                ...
                Clock.schedule_once(lambda dt, i=i: self.change_color(
                    self.beat_boxes[i], [1, 0, 0, 1]))
                sleep(beat_interval)
                Clock.schedule_once(lambda dt, i=i: self.change_color(
                    self.beat_boxes[i], [1, 1, 1, 1]))
